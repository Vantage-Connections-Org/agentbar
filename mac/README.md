...
- **Footer:** *Mark all finished as seen*, *Quit AgentBar*.

## Download

Every push to `main` that touches `mac/` builds `AgentBar-mac.zip` in GitHub Actions
(**Actions → mac → latest run → Artifacts**). Unzip it and move `AgentBar.app` to `/Applications`.

The app is **not signed with a Developer ID and not notarized**. The first time you open it:

- macOS 13–14: right-click `AgentBar.app` → **Open** → **Open**.
- macOS 15 and later: open it once, then go to **System Settings → Privacy & Security** and click **Open Anyway**.
- Or clear the quarantine flag: `xattr -dr com.apple.quarantine /Applications/AgentBar.app`
...
bash build-app.sh         # universal release build → build/AgentBar.app and build/AgentBar-mac.zip
open build/AgentBar.app
```

To start it at login, add `AgentBar.app` under **System Settings → General → Login Items**.

## How clicking finds the window

AgentBar walks up the process tree from the agent (`sysctl` `KERN_PROC`) to the nearest regular app,
...
- **Codex:** a chat counts as live while another process holds a lock on
  `~/.codex/thread-writer-locks/<id>.lock`. AgentBar tests this with a non-blocking `flock`. All Codex
...
| | |
|---|---|
| `Sources/AgentBar/Discovery.swift` | Finds the chats (port of `src/Discovery.cs`) |
| `Sources/AgentBar/Store.swift` | Polling, the waiting / seen / idle state machine, persistence, activating the host app |
| `Sources/AgentBar/AgentBarApp.swift` | Menu bar image and dropdown |
| `Sources/AgentBar/Icons.swift` | Claude and OpenAI logos, with a small SVG path parser |
| `Info.plist`, `build-app.sh` | App bundle (`LSUIElement`: menu bar only, no Dock icon) |

State (`seen.json`, `order.json`) is kept in `~/Library/Application Support/AgentBar/`.
